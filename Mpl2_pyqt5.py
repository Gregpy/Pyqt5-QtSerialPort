# Faster matplotlib version, but still slow, plot is updated rather than cleared and redrawn
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def update_plot(self, adc):
        ...
        if self._plot_ref is None: # if doesn't exist yet
            # First time we have no plot reference, so do a normal plot.
            # .plot returns a list of line <reference>s, as we're
            # only getting one we can take the first element.
            plot_refs = self.canvas.axes.plot(self.x, self.y, 'r')
            self._plot_ref = plot_refs[0] # dash before variable indicates it is private, by convention, or is internal, won't import with *
        else:
            # We have a reference, we can use it to update the data for that line.
            self.canvas.axes.set_ylim(0, max(self.y)+10)
            self._plot_ref.set_data(self.x, self.y) # updates data
