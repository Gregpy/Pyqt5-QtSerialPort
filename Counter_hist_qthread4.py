...
# most lines were explained in nogui, basic, timer or qthread(:3) geigercounting files
class GenericWorker(QtCore.QThread):
    #finished = QtCore.pyqtSignal() # may not be needed
...
# 
#     @QtCore.pyqtSlot() 
#     def run(self): # may not be needed
#         pass
# =============================================================================
# =============================================================================
#         try:
            ...
            try:
                ...
                if resp[:6] == 'DEBUG:':
                    ...
                    if resp == 'HELLO':
                        ...
                        self.worker = GenericWorker(self.wg.update_plot)
                        self.worker.result.connect(self.wg.update_plot)

                    if self.counter >= self.counter_comm + 4:

                        self.counts.append(float(resp))
                        self.worker.result.emit(self.counts)
