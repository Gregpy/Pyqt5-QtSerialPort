...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        button1 = QtWidgets.QPushButton("Send Integer Value", clicked = self.send)
    ...
    def send(self):
        try: 
            int(self.val.text())
            self.serial_port.write(self.val.text().encode())
        except:
            print('Please enter integer only', self.val.text())
