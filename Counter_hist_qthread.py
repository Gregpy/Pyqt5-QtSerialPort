# most lines were explained in nogui, basic, or timer geigercounting files
...
class Worker(QtCore.QObject): # worker object used to create signals, which send values to slots, between threads
    endmax = QtCore.pyqtSignal(int) # int signal used to notify plot when max data is reached and to end
    newcounts = QtCore.pyqtSignal(object) # object signal used to emit counts to the plot
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, verbose = 0, *args, **kwargs):
        ...
        self.thread = QtCore.QThread() # uses qthread which works with the pyqt eventloop
        self.worker = Worker() # instance of worker object
        self.worker.moveToThread(self.thread) # worker object is sent to thread (this is considered the proper way to thread with pyqt, though other ways work)
            ...
            try:
                ...
                if resp[:6] == 'DEBUG:':
                    ...
                    if resp == 'HELLO':
                        ...
                        self.wg = MainWindowg() # a plot window instance is created
                        self.wg.show() # the plot window is show
                        self.worker.newcounts.connect(self.wg.update_plot) # newcounts signal is connected with the update_plot slot method in the plot window 
                        self.worker.endmax.connect(self.wg.max_num) # endmax signal is connected with the max_num slot method in the plotting window
                        self.thread.start() # the plotting thread is started

                        self.worker.endmax.emit(self.intervals_value * self.replicas_value) # this emits the maximum number of data points to be used to plot
                    if self.counter >= self.counter_comm + 4:
                        
                        self.counts.append(float(resp))
                        self.worker.newcounts.emit(self.counts) # this emits count data to the plotting method

                    if len(self.counts) == (self.intervals_value * self.replicas_value):
                        if self.prints.checkState() == 2:
                            print(self.output.toPlainText())
                        self.thread.quit() # quit the plotting thread when ending
                        self.wg.close() # close the plotting window
                        self.close()
                        break
                    self.counter += 1
    ...
    def closeEvent(self, event):
        ...
        try:
            self.wg.close() # close the plotting window when closing the main window
        except:
            print('No plotting window started') # or print no plotting window started
        try:
...
class MainWindowg(QtWidgets.QMainWindow): # plotting window object

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs) 

        self.canvas = MplCanvas(self, width=5, height=4, dpi=100)
        self.setCentralWidget(self.canvas) # sets the matplotlib canvas as the center widget

        self.show() # show the plotting window

    @QtCore.pyqtSlot(object) # slot ensures object is sent by signal
    def update_plot(self, counts): # slot function run with counts
        self.canvas.axes.cla() # clears the plot
        ...
        if len(counts) == self.numdata:
             self.close() # close the window when the max number is reached
             
    @QtCore.pyqtSlot(int) # ensures int (decorators are not always needed for slots, which bascially wrap the following function in another)
    def max_num(self, maxnum): # slot function gets the maximum number emitted 
        self.numdata = maxnum # turned into a class attribute
