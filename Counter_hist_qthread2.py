...
# most lines were explained in nogui, basic, timer or qthread geigercounting files
class GenericWorker(QtCore.QObject): # worker class that can be applied to general scenarios
    #finished = QtCore.pyqtSignal() # used to finish process, may be needed
    start = QtCore.pyqtSignal(object) # signal used to start process
    def __init__(self, func): # initialize worker with function
        super().__init__() # initialize qobject

        self.func = func # func is set to worker attribute
    
    @QtCore.pyqtSlot(object) # ensures slot is object
    def run(self, cnts): # slot function run from initialized worker
        self.func(cnts) # function run with counts
        #self.finished.emit() # finished process signal emitted
            ...
            try:
                ...
                if resp[:6] == 'DEBUG:':
                    ...
                    if resp == 'HELLO':
                        ...
                        self.thread = QtCore.QThread() # thread created
                        self.wg = MainWindowg() # graphing window instance created
                        self.wg.show() # graphing window shown
                        self.worker = GenericWorker(self.wg.update_plot) # worker initialized with update plot function

                        self.worker.start.connect(self.worker.run) # start signal of worker connected to run function
                        self.worker.moveToThread(self.thread) # worker is moved to thread
                        self.thread.start() # thread is started

                    if self.counter >= self.counter_comm + 4:

                        self.counts.append(float(resp))
                        self.worker.start.emit(self.counts) # worker start signal emits with counts

                    if len(self.counts) == (self.intervals_value * self.replicas_value):
                        if self.prints.checkState() == 2:
                            print(self.output.toPlainText())
                        
                        self.wg.close() # close graphing window
                        self.close()
                        break
                    self.counter += 1
